  ...
  profileBloc(Emitter<ProfileState> emit, ProfileEvent event) async {
    ...
    try {
      if (event is GetProfileEvent) {
        ...
        if (event.userProfileModel.firstName == "") {
          emit(const ProfileStateFailure(error: "Họ không được để trống!"));
        } else if (event.userProfileModel.lastName == "") {
          emit(const ProfileStateFailure(error: "Tên không được để trống!"));
        } else {
